...
impl Stack {
    ...
    fn grow_stack(
        ...
        let new_page = Page::<Size4KiB>::containing_address(addr);
        let alloc_count = self.range.start - new_page;
        let old_count = self.range.end - self.range.start;
        elf::map_range(new_page.start_address().as_u64(), alloc_count, mapper, alloc);
        self.usage = old_count + alloc_count;
        self.range = Page::range(new_page, new_page + self.usage);
        info!("New page: {:?}, Range start: {:?}", new_page, self.range.start);
